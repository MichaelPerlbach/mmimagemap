		...
		if(count($res) > 0){
			foreach($res as $area){
				...
				if (!is_file($abs_apic)) {
					...
					if(preg_match("/WIN/", PHP_OS)) { $simg = ' "'.PATH_site.'typo3conf/ext/mmimagemap/Resources/Public/Images/canvas.png" -resize '.$imgsize[0].'x'.$imgsize[1].'!'; }
				}
				switch( intval($area['areatype']) ) {
					...
					case 0: //rectangle
						...
						if(preg_match("/WIN/", PHP_OS)) { exec('"'.rtrim($impath).'" convert -quality 100 '.$simg.' -stroke "rgb('.$bc.')" -strokewidth '.$area['feborderthickness'].' -fill none -draw " rectangle '.$points[2].','.$points[3].','.($points[2]+$points[0]).','.($points[3]+$points[1]).'" "'.$abs_apic.'"'); }
						else { exec($impath.'convert -quality 100 '.$simg.' -stroke "rgb('.$bc.')" -strokewidth '.$area['feborderthickness'].' -fill none -draw " rectangle '.$points[2].','.$points[3].','.($points[2]+$points[0]).','.($points[3]+$points[1]).'" '.$abs_apic); }
					...
					case 1: // circle
						...
						if(preg_match("/WIN/", PHP_OS)) { exec('"'.rtrim($impath).'" convert -quality 100 '.$simg.' -stroke "rgb('.$bc.')" -strokewidth '.$area['feborderthickness'].' -fill none -draw " circle '.$points[2].','.$points[3].','.($points[2]+$points[0]).','.($points[3]+$points[1]).'" "'.$abs_apic.'"'); }
						else { exec($impath.'convert -quality 100 '.$simg.' -stroke "rgb('.$bc.')" -strokewidth '.$area['feborderthickness'].' -fill none -draw " circle '.$points[2].','.$points[3].','.($points[2]+$points[0]).','.($points[3]+$points[1]).'" '.$abs_apic); }
					...
					case 2: // polygon
						...
						if(preg_match("/WIN/", PHP_OS)) { exec('"'.rtrim($impath).'" convert -quality 100 '.$simg.' -stroke "rgb('.$bc.')" -strokewidth '.$area['feborderthickness'].' -fill none -draw " polygon '.$points.'" "'.$abs_apic.'"'); }
						else { exec($impath.'convert -quality 100 '.$simg.' -stroke "rgb('.$bc.')" -strokewidth '.$area['feborderthickness'].' -fill none -draw " polygon '.$points.'" '.$abs_apic); }
				...
				if(file_exists($abs_apic)) { $map->setAltfile($apic); }
				else { $map->setAltfile(''); }
			...
			if(count($res) > 0){
				foreach($res as $area){
					...
					if (!is_file($ypic)) {
						...
						if(preg_match("/WIN/", PHP_OS)) { $timg = ' "'.PATH_site.'typo3conf/ext/mmimagemap/Resources/Public/Images/canvas.png" -resize '.$imgsize[0].'x'.$imgsize[1].'!'; }
					}
					...
					switch( intval($area['areatype'] )) {
						case 0: // rectangle
							...
							if(preg_match("/WIN/", PHP_OS)) { exec('"'.rtrim($impath).'" convert -quality 100 '.$timg.' -stroke "rgb('.$bc.')" -strokewidth '.$area['feborderthickness'].' -fill none -draw " rectangle '.$points[2].','.$points[3].','.($points[2]+$points[0]).','.($points[3]+$points[1]).'" "'.$ypic.'"'); }
							else { exec($impath.'convert -quality 100 '.$timg.' -stroke "rgb('.$bc.')" -strokewidth '.$area['feborderthickness'].' -fill none -draw " rectangle '.$points[2].','.$points[3].','.($points[2]+$points[0]).','.($points[3]+$points[1]).'" '.$ypic); }
						...
						case 1: // circle
							...
							if(preg_match("/WIN/", PHP_OS)) { exec('"'.rtrim($impath).'" convert -quality 100 '.$timg.' -stroke "rgb('.$bc.')" -strokewidth '.$area['feborderthickness'].' -fill none -draw " circle '.$points[2].','.$points[3].','.($points[2]+$points[0]).','.($points[3]+$points[1]).'" "'.$ypic.'"'); }
							else { exec($impath.'convert -quality 100 '.$timg.' -stroke "rgb('.$bc.')" -strokewidth '.$area['feborderthickness'].' -fill none -draw " circle '.$points[2].','.$points[3].','.($points[2]+$points[0]).','.($points[3]+$points[1]).'" '.$ypic); }
						...
						case 2: // polygon
							...
							if(preg_match("/WIN/", PHP_OS)) { exec('"'.rtrim($impath).'" convert -quality 100 '.$timg.' -stroke "rgb('.$bc.')" -strokewidth '.$area['feborderthickness'].' -fill none -draw " polygon '.$points.'" "'.$ypic.'"'); }
							else { exec($impath.'convert -quality 100 '.$timg.' -stroke "rgb('.$bc.')" -strokewidth '.$area['feborderthickness'].' -fill none -draw " polygon '.$points.'" '.$ypic); }
					...
					if(file_exists($oldimg)) { unlink($oldimg); }
					if(file_exists($ypic)) { $updatearea->setFealtfile($area['uid'].'_'.$apic); }
					else { $updatearea->setFealtfile(''); }
